...
public class TechnicianFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        logger.info("TechnicianFilter过滤器初始化");
    }

    @Override
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
        logger.info("进入TechnicianFilter过滤器,处理请求");
        ...
        //session验证
        if (request.getSession(false) != null && request.getSession(false).getAttribute("technician") != null){
            chain.doFilter(request,response);
        }else{
    ...
    @Override
    public void destroy() {
        logger.info("TechnicianFilter过滤器销毁");
    }
}
